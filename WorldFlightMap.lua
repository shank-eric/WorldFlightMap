...
local function GetArrow(pin)
	...
	if PinArrows[pin] then
		return PinArrows[pin]
	end
	...
	if event == "TAXIMAP_OPENED" then
		...
		if InCombatLockdown() then
			...
			if IsInInstance() then
				...
				if instanceID == 2657 or instanceID == 2481 or instanceID == 2965 then
...
function WorldFlightMapProvider:AddFlightNode(taxiNodeData)
	if self.taxiMap and self.worldMap and self.worldMap.left then
		...
		if self.worldMap.continent == self.playerContinent then
			...
			if
				self.worldMap.mapInfo
				and self.worldMap.mapInfo.mapType
				and self.worldMap.mapInfo.mapType > 2
				and taxiNodeData.state == Enum.FlightPathState.Reachable
			then
...
function WorldFlightMapProvider:HighlightRouteToPin(pin)
	if self.playerContinent == 905 then
		return
	end -- don't draw lines on argus maps (we could if they're on the same zone map)
